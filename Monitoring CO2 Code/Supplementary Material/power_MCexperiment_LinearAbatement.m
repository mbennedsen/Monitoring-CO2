...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Linear Abatement: Monte Carlo power experiments.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for iM = 1:length(m_vec)
    ...
    %E_act_slope = E_ast_slope*(1-m_vec(iM));
    f = m_vec(iM);
    ...
    for iMC = 1:MC
        ...
        while detect_period < 0 && t < T
            ...
            for it = 1:(K+t)
                ...
                if it>tau-1
                    t_break = t_break+1;
                    E_path_act = (1-f)*max(E_ANT(end) - E_ast_slope*t_break,0) + f*E_ANT(end); 
                    E_path_ast = max(E_ANT(end) - E_ast_slope*t_break,0);
...
%% Save
save(['Files/power_linAbate_DGP',num2str(DGP),'_K',num2str(K),'_T',num2str(T),'_v01'],'twoSide_res','oneSide_res','oneSide_det','MC');
